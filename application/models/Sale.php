...
class Sale extends CI_Model
{
	...
	public function get_info($sale_id)
	{
		...
		$this->db->select('
				...
				sales.payment_status, sales.total_order,
		');
	...
	public function update($sale_id, $sale_data, $payments)
	{
		...
		if ($success && !empty($payments)) {
			...
			$payment_status = $this->get_sales_payment_status($sale_id);
			$this->update_sale_payment_status($sale_id,$payment_status);
		}
		return $success;
	}
	...
	public function update_sale_payment_status($sale_id, $sale_payment_status)
	{
		$this->db->where('sale_id', $sale_id);
		$this->db->update('sales', array('payment_status' => $sale_payment_status));
	}
	...
	public function get_sales_payment_status($sales_id = null){
		$sale_status = 0;
		if (!is_null($sales_id)){
			$sales_payment = $this->get_sale_payments($sales_id)->result();
			$total_paid = 0;
			foreach ($sales_payment as $idx => $sale_payment){
				if ($sale_payment->payment_type != 'Due'){
					$total_paid = $total_paid + $sale_payment->payment_amount;
				}
			}
	...
	function get_sales_return_order_summary($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sales_items.supplier_id', $order = 'asc', $count_only = FALSE, $supplier_id = null){
		...
		$str_field = 'items.item_id, items.item_number, items.name, sales_items.supplier_id, sales.sale_type, ';
		$str_field .= 'SUM(quantity_purchased) AS total_qty, ';
		...
		$this->db->group_by('sales_items.supplier_id');
		$this->db->group_by('sales_items.item_id');
		$this->db->group_by('sales.sale_type');
		if($count_only == TRUE) {
			return $this->db->get()->num_rows();
		}
		// order by sale time by default
	...
	public function get_found_sales_summary_rows($search, $filters)
	{
		return $this->get_sales_return_order_summary($search, $filters, 0, 0, 'sales.sale_time', 'desc', TRUE);
	}
}
?>
